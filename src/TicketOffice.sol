...
contract TicketOffice is ITicketOffice, ReentrancyGuard {
  using TicketStructs for TicketStructs.Ticketdetails;
  using TicketStructs for TicketStructs.LocationDetails;

  IERC20 immutable internal _usdc;
  string internal _name;
  address immutable internal _contractOwner;
  ...
  mapping(address => mapping(uint256 => mapping(uint256 => bool))) public ticketComped;
  ...
  modifier okAdmin(uint256 eventId) {
    require(
      getEventOwner(eventId) == msg.sender || withdrawlApproval[eventId] == msg.sender,
      "Unauthorized user"
    );
    _;
  }
  ...
  function getTicketsSold(uint256 eventId) public view returns (uint256[] memory) {
    return eventDetails[eventId].ticketInformation.ticketsSold;
  }
  ...
  {
    require(
      block.timestamp + 86_400 * 3 < details.eventDate,
      "Event Must be created at least 3 days in advance"
    );
    ...
    nonReentrant
  {
    ...
    require(
      eventDetails[eventId].ticketInformation.ticketCapacities[ticketId]
        >= quantity + eventDetails[eventId].ticketInformation.ticketsSold[ticketId],
      "Event is Sold out"
    );
    address id = eventTicketAddress[eventId];
    ...
    nonReentrant
  {
    ...
    for (uint256 i = 0; i < length; i++) {
      require(
        eventDetails[eventId].ticketInformation.ticketCapacities[i]
          >= amounts[i] + eventDetails[eventId].ticketInformation.ticketsSold[i],
        "Event is Sold out"
      );
  ...
  function redeemTicket(uint256 eventId, uint256 ticketId) public openOffice {
    ...
    eventDetails[eventId].ticketInformation.ticketsSold[ticketId] += 1;
    ticketComped[msg.sender][eventId][ticketId] = true;
  }

  function issueRefund(uint256 eventId) public openOffice nonReentrant {
    require(eventLocked[eventId], "Refund Ineligible");
    address id = eventTicketAddress[eventId];
    uint256 refundBalance = 0;
    uint256 userBalance;
    for (uint256 i = 0; i < eventDetails[eventId].ticketInformation.ticketNames.length; i++) {require(!ticketComped[msg.sender][eventId][i], "Refund Ineligible");
      userBalance = ERC1155Token(id).balanceOf(msg.sender, i);
      if (userBalance > 0) {
        ERC1155Token(id).burn(msg.sender, i, userBalance);
        require(ERC1155Token(id).balanceOf(msg.sender, i) == 0, "Token Burn Failed");
        eventBalance[eventId] -=
          eventDetails[eventId].ticketInformation.ticketPrices[i] * userBalance;
        eventDetails[eventId].ticketInformation.ticketsSold[i] -= userBalance;
        refundBalance += eventDetails[eventId].ticketInformation.ticketPrices[i] * userBalance;
      }
    }

     bool success = _usdc.transfer(msg.sender, refundBalance);
    require(success, "Transfer Failed");
  }

  function lockEvent(uint256 eventId) public openOffice {
    require(
      _contractOwner == msg.sender || getEventOwner(eventId) == msg.sender
        || withdrawlApproval[eventId] == msg.sender,
      "Unauthorized Access"
    );
    ...
    openOffice
  {
    require(!eventLocked[eventId], "Concert Event is Frozen");
    require(
      eventDetails[eventId].eventDate - 172_800 >= block.timestamp,
      "Date change within Locked Period"
    );
    require(details.eventDate > block.timestamp, "Invalid Date");
    require(eventDetails[eventId].owner == details.owner, "Unauthorized OwnerSwap");

    eventDetails[eventId] = TicketStructs.Ticketdetails(
      details.name,
      msg.sender,
      details.eventDate,
      eventDetails[eventId].ticketInformation,
      details.locationDetails,
      details.performers,
      details.keywords,
      details.categories,
  ...
  function withdrawFunds(uint256 eventId) public openOffice nonReentrant {
    require(
      block.timestamp - 86_400 >= eventDetails[eventId].eventDate,
      "Withdraw must be one day after Event"
    );
    require(!eventLocked[eventId], "Funds are locked for refunds");
    require(eventTicketAddress[eventId] != address(0), "Event Does Not Exist");

    require(eventBalance[eventId] > 0, "No Funds to Withdraw");
    require(
      msg.sender == eventDetails[eventId].owner || msg.sender == withdrawlApproval[eventId],
      "Unauthorized Access"
    );
    ...
    eventLocked[eventId] = true;
    bool success1 = _usdc.transfer(_contractOwner, fee);
    require(success1, "Transfer Failed");
    bool success2 = _usdc.transfer(eventDetails[eventId].owner, payOut);
    require(success2, "Transfer Failed");
  }

  function revokeTickets(
    address user,
    uint256 eventId,
    uint8 tokenId,
    uint256 value
  ...
  function emitEvent(uint256 eventId) internal {
    emit Event(
      ...
      eventDetails[eventId].eventDescription,
